...
class LLM_Store(
    ...
    async def _disambiguate(
        ...
        # PATCH 3 (weather-graph vendor): upstream imports this from the *top-level* `kbel`
        # package, an undeclared runtime dependency (blocker #6), and asks for
        # `LLM_Disambiguator` -- a name it never uses. Only the `Disambiguator('llm', ...)` call
        # below needs the 'llm' plugin registered, and that branch is unreachable unless
        # `entity_linking_method=EntityLinkingMethod.LLM`, which this repo never sets. So the
        # import is made relative to the vendored `kbel` subset and `LLM_Disambiguator` (whose
        # module was deliberately not vendored) is dropped. `SimpleDisambiguator` stays: importing
        # it is what registers the 'simple' plugin via `Disambiguator.__init_subclass__`.
        from ..kbel.disambiguators import (
            Disambiguator,
            SimpleDisambiguator,  # noqa: F401 - imported for its plugin-registration side effect
        )
