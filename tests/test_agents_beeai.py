"""Generation + tool tests for the opt-in BeeAI + Mellea backend (no LLM / no network).

Skips cleanly (via the `mock_beeai_generation` fixture) when `mellea`/`beeai-framework` aren't
installed — they're the optional `beeai` extra (`uv sync --extra beeai`), demoted out of the
default install. See `tests/test_agents_shared.py` for the equivalent matrix run against the
pydantic-ai/LangGraph backends' shared contract instead.
"""
...
def test_generate_spec_returns_valid(mock_beeai_generation):
    from weather_graph.agents.beeai_generation import generate_spec

    mock_beeai_generation.queue(_spec(VALID_SPARQL))
    spec = generate_spec("What is the weather in Paris?")
    assert spec.sparql == VALID_SPARQL
    assert len(mock_beeai_generation.calls) == 1
    # Requirements are actually plumbed through to the generative call.
    assert mock_beeai_generation.calls[0]["requirements"]


def test_generate_spec_repairs_invalid_then_valid(mock_beeai_generation):
    from weather_graph.agents.beeai_generation import generate_spec

    mock_beeai_generation.queue(_spec(INVALID_NO_LIMIT), _spec(VALID_SPARQL))
    spec = generate_spec("Weather in Paris?")
    assert spec.sparql == VALID_SPARQL
    # Two calls: the initial one and one repair.
    assert len(mock_beeai_generation.calls) == 2
    # The repair prompt tells the model what was wrong.
    assert "rejected" in mock_beeai_generation.calls[1]["question"].lower()
    assert "LIMIT" in mock_beeai_generation.calls[1]["question"]


def test_generate_spec_gives_up_after_max_repairs(mock_beeai_generation):
    from weather_graph.agents.beeai_generation import generate_spec

    mock_beeai_generation.queue(_spec(INVALID_NO_LIMIT), _spec(INVALID_NO_LIMIT))
    with pytest.raises(SparqlValidationError, match="LIMIT"):
        generate_spec("Weather in Paris?", max_repairs=1)
    assert len(mock_beeai_generation.calls) == 2


def test_answer_question_end_to_end(mock_beeai_generation):
    from weather_graph.agents.beeai_tools import answer_question

    mock_beeai_generation.queue(_spec(VALID_SPARQL, intent="Paris temperature"))
...
def test_answer_question_retries_on_empty_categorical_result(mock_beeai_generation):
    ...
    assert len(mock_beeai_generation.calls) == 2
    retry_question = mock_beeai_generation.calls[1]["question"]
    assert "Rainy" in retry_question and "Snowy" in retry_question


def test_answer_question_no_retry_without_categorical_filter(mock_beeai_generation):
    """A genuinely empty result with no categorical string filter must not trigger a retry."""
    from weather_graph.agents.beeai_tools import answer_question

    mock_beeai_generation.queue(_spec(NO_MATCH_NUMERIC, intent="no match"))
    payload = answer_question("Any city over 1000 degrees?")
    assert payload["rows"] == []
    assert len(mock_beeai_generation.calls) == 1


def test_answer_question_retries_on_guessed_city_uri(mock_beeai_generation):
    ...
    assert len(mock_beeai_generation.calls) == 2
    assert "wx:name" in mock_beeai_generation.calls[1]["question"]
